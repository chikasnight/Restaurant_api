...
class UploadImage implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            // Original
            if (Storage::disk($disk)->put('/uploads/food/original/' . $imageName, fopen($original_file, 'r+'))) {
                File::delete($original_file);
            }

            // Large
            if (Storage::disk($disk)->put('/uploads/food/large/' . $imageName, fopen($large, 'r+'))) {
                File::delete($large);
            }

            // Thumbnail
            if (Storage::disk($disk)->put('/uploads/food/thumbnail/' . $imageName, fopen($thumbnail, 'r+'))) {
